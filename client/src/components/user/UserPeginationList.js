...
function UserPeginationList() {
  const navigate = useNavigate();
  const { loading, error, data, fetchMore } = useQuery(SERVER_PEGINATION, {
    variables: {
      offset: 0,
      limit: 4,
    },
  ...
  const usersdata = loading
    ? []
    : data.peginationUser.map((user, i) => ({
        Number: i + 1,
        ID: user.id,
...
export default UserPeginationList;
